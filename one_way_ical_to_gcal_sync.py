...
def create_gcal_event_from_ical_event(ical_event):
    gcal_event = {}
    ...
    gcal_event["source"] = {
        "title": "Imported from one_way_ical_to_gcal_sync.py",
        "url": url_feed,
    }
...
if __name__ == "__main__":
    ...
    # datetime instance representing the start of the current day (UTC)
    ...
    for feed in config.get("ICAL_FEEDS"):
        ...
        # TODO this was tweaked so events would not get deleted
        ...
        for gcal_event in gcal_events:
            ...
            if eid not in ical_events:
                pass
                # # TODO this was commented out so things would not get deleted
        ...
        for ical_id, ical_event in ical_events.items():
            if ical_id not in gcal_event_ids:
                gcal_event = create_gcal_event_from_ical_event(ical_event)
